...
impl EastmoneyLsjzProvider {
    pub async fn fetch_by_date(&self, code: &str, date: &str) -> Result<FundData, String> {
        ...
        let json_str = if body.starts_with("jQuery") {
            let caps = JSONP_RE.captures(&body).ok_or("Failed to match JSONP pattern in lsjz")?;
            caps.get(1).ok_or("Failed to extract JSON from JSONP in lsjz")?.as_str().to_string()
        } else {
            body
        };

        let res: LsjzResponse = serde_json::from_str(&json_str).map_err(|e| format!("{}: {}", e, json_str))?;
        let items = res.Data.ok_or("No Data in lsjz response")?.LSJZList;
        
        let item = items.into_iter().find(|i| i.FSRQ == date)
