...
pub async fn resolve_fund(conn: &Connection, input: &str) -> Result<Fund, String> {
    ...
    if is_6_digit_code(input) {
        println!("✨ '{}' looks like a fund code. Trying direct fetch...", input);
        match sync_fund_details(conn, input).await {
            Ok(fund) => return Ok(fund),
            Err(e) => return Err(format!("Failed to fetch fund details for '{}': {}", input, e)),
        }
    }

    // 3. Fuzzy search remotely
    println!("🔍 Fund '{}' not found locally. Searching remotely...", input);
...
pub async fn sync_fund_details(conn: &Connection, code: &str) -> Result<Fund, String> {
    ...
    aggregator.add_provider(Box::new(crate::provider::eastmoney_details::EastmoneyDetailProvider));
    ...
    if let (Some(nav), Some(date)) = (data.nav, data.date) {
        db::insert_nav_history_idempotent(conn, &data.code, &date, &nav.to_string()).map_err(|e| e.to_string())?;
    }

    db::get_fund_by_code_or_name(conn, code).map_err(|e| e.to_string())?
