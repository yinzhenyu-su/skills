...
func (fs *QryptFS) Release(path string, fh uint64) (errc int) {
	...
	logging.L.Infof("[FUSE] Release: path=%s, fh=%d\n", path, fh)
...
func (fs *QryptFS) Fsync(path string, datasync bool, fh uint64) (errc int) {
	logging.L.Infof("[FUSE] Fsync: path=%s, datasync=%v, fh=%d\n", path, datasync, fh)
	node, errc := fs.lookup(path)
	if errc != 0 {
		return errc
	}
...
func (fs *QryptFS) Ftruncate(path string, size int64, fh uint64) (errc int) {
	logging.L.Infof("[FUSE] Ftruncate: path=%s, size=%d, fh=%d\n", path, size, fh)
...
func (fs *QryptFS) Init() {
	logging.L.Infof("[FUSE] Init: QryptFS starting up\n")
}

func (fs *QryptFS) Destroy() {
	logging.L.Infof("[FUSE] Destroy: QryptFS shutting down\n")
}
