...
func (fs *QryptFS) storeNode(path string, n *Node) {
	...
	logging.L.Debugf("storeNode: path=%s fid=%s isFolder=%v\n", path, n.fid, n.isFolder)
...
func (fs *QryptFS) deleteNodePath(path string, n *Node) {
	...
	logging.L.Debugf("deleteNodePath: path=%s fid=%s isFolder=%v\n", path, n.fid, n.isFolder)
	...
	if path != "/" {
		...
		if v, ok := fs.nodes.Load(parentPath); ok {
			...
			p.mu.Lock()
			if p.children != nil {
				logging.L.Debugf("deleteNodePath: removing child %s from parent %s\n", filepath.Base(path), parentPath)
				delete(p.children, filepath.Base(path))
			}
			p.mtime = time.Now()
			p.mu.Unlock()
		} else {
			logging.L.Debugf("deleteNodePath: parent %s not in memory (already removed?)\n", parentPath)
		}
	}
	...
	if n != nil {
		...
		if localPath != "" && fs.staging != nil {
			logging.L.Debugf("deleteNodePath: removing staging %s\n", localPath)
	...
	logging.L.Debugf("deleteNodePath: done path=%s\n", path)
}
...
func (fs *QryptFS) deleteSubtreePaths(parentPath string, n *Node) {
	...
	logging.L.Debugf("deleteSubtreePaths: start path=%s\n", parentPath)
	...
	logging.L.Debugf("deleteSubtreePaths: %d children to clean up under %s\n", childCount, parentPath)

	var fidsToPurge []string

	for _, c := range children {
		childPath := prefix + c.name
		logging.L.Debugf("deleteSubtreePaths: processing child %s\n", childPath)
	...
	if fs.cacheMgr != nil && len(fidsToPurge) > 0 {
		logging.L.Debugf("deleteSubtreePaths: purging %d fids\n", len(fidsToPurge))
		fs.cacheMgr.BatchDeleteNodeState(fidsToPurge, nil)
	}
	logging.L.Debugf("deleteSubtreePaths: done path=%s\n", parentPath)
}
...
func (fs *QryptFS) fetchFiles(fid string) ([]backend.Entry, error) {
	...
	if v, ok := fs.fetchingFiles.Load(fid); ok {
		...
		<-r.done
		logging.L.Debugf("fetchFiles: waited for concurrent fetch of %s (took %v)\n", fid, time.Since(start))
	...
	close(r.done)
	logging.L.Debugf("fetchFiles: done fid=%s got %d files err=%v (took %v)\n", fid, len(r.files), r.err, time.Since(start))
	return r.files, r.err
}
