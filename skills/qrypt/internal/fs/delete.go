...
func (fs *QryptFS) Unlink(path string) (errc int) {
	defer func() {
		if r := recover(); r != nil {
			log.L.Errorf("PANIC in Unlink(%s): %v\n", path, r)
			errc = -fuse.EIO
		}
	}()
	if fs.IsShuttingDown() {
		log.L.Warnf("[SHUTDOWN] Rejecting Unlink: %s\n", path)
		return -fuse.EIO
	}
	log.L.Infof("[FUSE] Unlink: path=%s\n", path)
...
func (fs *QryptFS) rmdirNonEmpty(path string, n *Node, parentFid string) int {
	log.L.Infof("Rmdir: %s is NOT empty, deleting children recursively\n", path)
	...
	fs.deleteNodePath(path, n)
	log.L.Infof("Rmdir: done deleting %s and %d children\n", path, len(allNodes))
	return 0
}
...
func (fs *QryptFS) Rmdir(path string) (errc int) {
	defer func() {
		if r := recover(); r != nil {
			log.L.Errorf("PANIC in Rmdir(%s): %v\n", path, r)
			errc = -fuse.EIO
		}
	}()
	if fs.IsShuttingDown() {
		log.L.Warnf("[SHUTDOWN] Rejecting Rmdir: %s\n", path)
		return -fuse.EIO
	}
	log.L.Infof("[FUSE] Rmdir: path=%s\n", path)
