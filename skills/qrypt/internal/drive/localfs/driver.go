...
var (
	...
	_ drive.Writer   = (*LocalDriver)(nil)
	_ drive.Uploader = (*LocalDriver)(nil)
)
...
func (d *LocalDriver) List(ctx context.Context, parentID string) ([]drive.Entry, error) {
	...
	for _, e := range entries {
		...
		result = append(result, drive.Entry{
...
func (d *LocalDriver) Read(ctx context.Context, entry drive.Entry, offset, size int64) (io.ReadCloser, error) {
...
func (d *LocalDriver) Mkdir(ctx context.Context, parentID, name string) (drive.Entry, error) {
	parent := d.resolve(parentID)
	path := filepath.Join(parent, name)
	err := os.Mkdir(path, 0755)
	if err != nil {
		return drive.Entry{}, fmt.Errorf("localfs: mkdir %s: %w", path, err)
	}
	return drive.Entry{ID: path, Name: name, IsDir: true, ModTime: time.Now()}, nil
}

func (d *LocalDriver) Move(ctx context.Context, entry drive.Entry, dstParentID string) error {
	dst := filepath.Join(d.resolve(dstParentID), filepath.Base(entry.ID))
	return os.Rename(entry.ID, dst)
}

func (d *LocalDriver) Rename(ctx context.Context, entry drive.Entry, newName string) error {
	parent := filepath.Dir(entry.ID)
	dst := filepath.Join(parent, newName)
	return os.Rename(entry.ID, dst)
}

func (d *LocalDriver) Remove(ctx context.Context, entry drive.Entry) error {
	if entry.IsDir {
...
func (d *LocalDriver) Put(ctx context.Context, parentID, name string, size int64, body io.Reader) (drive.Entry, error) {
	parent := d.resolve(parentID)
	path := filepath.Join(parent, name)
	f, err := os.Create(path)
	if err != nil {
		return drive.Entry{}, fmt.Errorf("localfs: create %s: %w", path, err)
	}
	defer f.Close()
	_, err = io.Copy(f, body)
	if err != nil {
		return drive.Entry{}, fmt.Errorf("localfs: write %s: %w", path, err)
	}
	info, err := f.Stat()
	if err != nil {
		return drive.Entry{ID: path, Name: name}, nil
	}
	return drive.Entry{ID: path, Name: name, Size: info.Size(), ModTime: info.ModTime()}, nil
}
