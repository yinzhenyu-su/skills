...
var (
	_ drive.Driver   = (*MockDriver)(nil)
	_ drive.Writer   = (*MockDriver)(nil)
	_ drive.Uploader = (*MockDriver)(nil)
)
...
func (d *MockDriver) List(ctx context.Context, parentID string) ([]drive.Entry, error) {
	d.mu.RLock()
	defer d.mu.RUnlock()
	var result []drive.Entry
...
func (d *MockDriver) Read(ctx context.Context, entry drive.Entry, offset, size int64) (io.ReadCloser, error) {
	d.mu.RLock()
...
func (d *MockDriver) Mkdir(ctx context.Context, parentID, name string) (drive.Entry, error) {
	d.mu.Lock()
	defer d.mu.Unlock()
	id := d.allocID()
	d.files[id] = &memFile{
	...
	return drive.Entry{ID: id, Name: name, IsDir: true, ModTime: time.Now()}, nil
}

func (d *MockDriver) Move(ctx context.Context, entry drive.Entry, dstParentID string) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	f, ok := d.files[entry.ID]
	if !ok {
...
func (d *MockDriver) Rename(ctx context.Context, entry drive.Entry, newName string) error {
	d.mu.Lock()
	defer d.mu.Unlock()
	f, ok := d.files[entry.ID]
	if !ok {
...
func (d *MockDriver) Remove(ctx context.Context, entry drive.Entry) error {
	d.mu.Lock()
	defer d.mu.Unlock()
...
func (d *MockDriver) Put(ctx context.Context, parentID, name string, size int64, body io.Reader) (drive.Entry, error) {
	...
	return drive.Entry{ID: id, Name: name, Size: int64(len(data)), ModTime: time.Now()}, nil
}
