...
var (
	_ drive.Driver   = (*Yun139Driver)(nil)
	_ drive.Writer   = (*Yun139Driver)(nil)
	_ drive.Uploader = (*Yun139Driver)(nil)
)
...
func (d *Yun139Driver) List(ctx context.Context, parentID string) ([]drive.Entry, error) {
	fileID := parentID
...
func (d *Yun139Driver) Read(ctx context.Context, entry drive.Entry, offset, size int64) (io.ReadCloser, error) {
...
func (d *Yun139Driver) Mkdir(ctx context.Context, parentID, name string) (drive.Entry, error) {
	fileID := parentID
	if fileID == "" || fileID == "0" || fileID == "/" {
		fileID = d.rootID
	}
	...
	if !resp.Success {
		return drive.Entry{}, fmt.Errorf("139 mkdir failed: %s", resp.Message)
	}
...
func (d *Yun139Driver) Remove(ctx context.Context, entry drive.Entry) error {
	data := map[string]interface{}{
		"fileIds": []string{entry.ID},
	}
	var resp baseResp
...
func (d *Yun139Driver) Put(ctx context.Context, parentID, name string, size int64, body io.Reader) (drive.Entry, error) {
	fileID := parentID
	if fileID == "" || fileID == "0" || fileID == "/" {
		fileID = d.rootID
	}
	...
	if err != nil {
		return drive.Entry{}, fmt.Errorf("139 upload: read: %w", err)
	}
	...
	if err != nil {
		return drive.Entry{}, fmt.Errorf("139 upload init: %w", err)
	}
	if !preResp.Success {
		return drive.Entry{}, fmt.Errorf("139 upload init failed: %s", preResp.Message)
	}
	...
	for i := 0; i < totalParts; i++ {
		...
		if uploadURL == "" {
			return drive.Entry{}, fmt.Errorf("139 upload: no url for part %d", partNum)
		}
		...
		if resp.StatusCode < 200 || resp.StatusCode >= 300 {
			return drive.Entry{}, fmt.Errorf("139 upload part %d: status %d", partNum, resp.StatusCode)
		}
	}
	...
	if !commitResp.Success {
		return drive.Entry{}, fmt.Errorf("139 upload complete failed: %s", commitResp.Message)
	}

	return drive.Entry{ID: commitResp.Data.FileId, Name: name, Size: size}, nil
}
