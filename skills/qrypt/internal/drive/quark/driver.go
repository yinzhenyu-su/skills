...
var (
	_ drive.Driver   = (*QuarkDriver)(nil)
	_ drive.Writer   = (*QuarkDriver)(nil)
	_ drive.Uploader = (*QuarkDriver)(nil)
)
...
func (d *QuarkDriver) List(ctx context.Context, parentID string) ([]drive.Entry, error) {
...
func (d *QuarkDriver) Read(ctx context.Context, entry drive.Entry, offset, size int64) (io.ReadCloser, error) {
	fid := entry.ID
...
func apiError(resp resp) error {
	...
	case 23008:
		return drive.ErrDirAlreadyExists
	}
	return fmt.Errorf("api error: status=%d code=%d msg=%s", resp.Status, resp.Code, resp.Message)
}

func (d *QuarkDriver) Mkdir(ctx context.Context, parentID, name string) (drive.Entry, error) {
	data := map[string]interface{}{
		"pdir_fid":      parentID,
		"file_name":     name,
		"dir_path":      "",
	...
	if err != nil {
		return drive.Entry{}, fmt.Errorf("mkdir: %w", err)
	}
	if rerr := apiError(resp.resp); rerr != nil {
		return drive.Entry{}, rerr
	}
...
func (d *QuarkDriver) Move(ctx context.Context, entry drive.Entry, dstParentID string) error {
	data := map[string]interface{}{
...
func (d *QuarkDriver) Rename(ctx context.Context, entry drive.Entry, newName string) error {
	data := map[string]interface{}{
		"fid":       entry.ID,
		"file_name": newName,
	}
...
func (d *QuarkDriver) Remove(ctx context.Context, entry drive.Entry) error {
	data := map[string]interface{}{
		"action_type":  1,
		"exclude_fids": []string{},
		"filelist":     []string{entry.ID},
...
func (d *QuarkDriver) Put(ctx context.Context, parentID, name string, size int64, body io.Reader) (drive.Entry, error) {
	...
	if err != nil {
		return drive.Entry{}, fmt.Errorf("upload pre: %w", err)
	}
	...
	if err != nil {
		return drive.Entry{}, fmt.Errorf("upload: read body: %w", err)
	}
	...
	if err != nil {
		return drive.Entry{}, fmt.Errorf("upload hash: %w", err)
	}
...
func (d *QuarkDriver) deleteExistingFileByName(parentFid, name string) {
	...
	if err != nil {
		log.L.Warnf("deleteExistingFileByName: list parent %s: %v", parentFid, err)
		return
	}
	for _, f := range files {
		if f.Name == name && !f.IsDir {
			_ = d.Remove(context.Background(), f)
			log.L.Infof("deleteExistingFileByName: removed existing %s (%s)", name, f.ID)
			return
		}
	}
}
