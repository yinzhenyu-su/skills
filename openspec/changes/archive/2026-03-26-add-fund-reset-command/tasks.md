...
- [x] 1.1 在 `src/cli.rs` 的 `FundCommands` 枚举中添加 `Reset` 变体
...
- [x] 3.1 在 `src/main.rs` 添加 `Commands::Fund { command: FundCommands::Reset }` 分支
...
- [x] 4.1 添加 CLI 集成测试：验证 `fund reset -y` 成功重置数据
